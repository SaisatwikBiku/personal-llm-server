...
Then fill in You, Application profile in the panel: every field application forms ask for, grouped as Name, Contact, Links, Work authorization, Experience, Education, Job preferences and optional self-identification, built from the questions on real application forms. Below the fields it lists the questions it still can't answer, most common first; answer one once and every form that asks it gets that answer. An empty field makes the matching form question show up as one to answer yourself. Legal consents are never answered for you. Edit `config.json` to change roles, companies (board names as they appear in the job board URLs; for Workday, `<tenant>.wdN/<site>` from a URL like `https://mtb.wd5.myworkdayjobs.com/MTB/job/...`, and for SmartRecruiters the company name from `https://jobs.smartrecruiters.com/<company>/...`), the run and digest times, and the score thresholds; the next run picks up the change without a restart. The first run happens at the next `run_at` time, or right away from Run now in the Jobs view.
...
To give the agent its own inbox, make a new Yahoo or Gmail address and create an app password for it (Yahoo: Account Info, Account Security, Generate app password; Gmail: turn on 2-step verification, then Google Account, Security, App passwords), and connect it in the Inbox tab. It becomes the email on your applications. Every 5 minutes the panel reads new mail without marking it read: an "application received" email marks the job Applied, interview requests and rejections move the job to Interviewing or Rejected, and verification codes appear in the list and as an alert. Email text is shown as plain text, never given to the model, and its links are never opened.

Each morning the Jobs view opens on Review: up to 50 prepared applications (`queue_size`) on Greenhouse, Lever and Ashby, best first. Open one, check the answers, fill in what's missing, tick the statements you agree to (each consent has its own box, or use Agree to all), and tap Approve. Then tap Apply to approved: it opens each approved form in your browser, fills it, ticks the consents you agreed to, and submits it after a 3-second countdown you can stop. After the confirmation it marks the job Applied and opens the next one. It stops and hands over when a CAPTCHA appears, a required question has no answer, or no confirmation comes; fix it and tap the form's Submit button and it carries on, or skip that job for now.

For a tailored resume and cover letter per job, put your resume in the jobs folder as `resume.json` (see `deploy/jobs-resume.example.json`; owned by `agentd`, mode 600). Each night, for the jobs it can submit, the agent orders your projects and skills by what the job asks for and rewrites the summary, without changing your bullets. It also drafts a cover letter. A summary that claims a skill your resume doesn't list is thrown away, and a cover letter that does gets a warning on the job. Open both PDFs from the job's Resume and cover letter section, edit the summary or the letter there, and approve. The autofill attaches them to the matching upload fields. The Review list only shows applications with all three ready: answers, resume and cover letter. Each night the agent gets the best matches ready first, and stops at 7:50 so the list is settled for the 8:00 alert. Matches still being prepared are counted at the top of Review, and Get them ready now does it on demand. Every application is also checked against hiring rules: the same posting twice or a repost, too many at one company (3 in 30 days by default, Google 3 in 90), unrelated roles at one company, reapplying within 180 days of a rejection after interviews, applying while interviewing there, graduation windows you're outside of, citizenship or clearance requirements, no sponsorship, different answers or resumes to one company, AI-use policies, and referrals or agency submissions you noted. Blocked jobs stay out of Review and Apply with the reason shown; others ask you to confirm with Approve anyway or show a note. Set each rule and the per-company caps under You, Rules. [HIRING_RULES.md](HIRING_RULES.md) lists every rule, what breaks it and its default action. A running search can be stopped with the stop button in the Jobs header.
