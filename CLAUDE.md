...
1. Any tool with side effects or outbound network access needs Sai's approval. `web_search` and `fetch_url` stay approval-gated because queries and URLs can carry data out. The one exception is `jobs.py`, which Sai approved on 2026-09-25: it sends unapproved requests, but only to URLs its own code builds from `config.json` or from board names parsed out of search results: the Greenhouse, Lever and Ashby board APIs, `api.smartrecruiters.com`, Workday tenants at `<tenant>.wdN.myworkdayjobs.com` (board names must match `WORKDAY_RE`; Workday search is a POST whose body holds only the configured search terms), and the local SearXNG. Model output must never become a URL, query or request body there, search results are reduced to a board name matched against those hosts, and nothing from `profile.json` or `resume.txt` may be sent anywhere. Keep it that way, and never make it submit applications.
