...
- Tailored documents (2026-09-26): with `resume.json` in place and `tailor_docs` on, the nightly run (and "Prepare answers", and the Make button on a job) calls `tailor()` for jobs the autofill can submit and stores `job["docs"]`: `projects` (indices, ordered by how many of the job's required skills each mentions, at most `MAX_PROJECTS`), `skills` (each group reordered, never added to), `summary` (the 8B's rewrite, kept only if 150 to 700 characters and `unsupported()` finds no skill the resume lacks; otherwise the resume's own summary), `cover` (the 8B's letter) and `warnings` (skills the letter claims that the resume lacks). Bullets are never rewritten. `unsupported()` checks the job's missing skills plus `TECH_TERMS`, because checking only the job's skills let "writes Go daily" through on a job that didn't ask for Go. The PDFs are rendered by reportlab each time they're requested (`doc_file()`, `render_resume()`, `render_cover()`), from `resume.json` plus `docs`, with the profile's email and phone in the header so employers write to the agent inbox; nothing is stored as a file. Built-in Helvetica covers Windows-1252 only, so `pdf_text()` swaps arrows and other characters outside it. `GET /api/jobs/doc` shows them in the panel; `GET /api/jobs/resume?id=&kind=` hands them to the userscript, which sends each upload field's id (Stripe labels both uploads "Attach"; the ids are `resume` and `cover_letter`) so `fill()` can pick the document, and puts the letter's text into cover letter text boxes. `autofillUpload()` skips an upload that sits by "autofill" text when the form has another upload: Ashby's "Autofill from resume" box parses the file and refills the form after the script has filled it, which wiped Phone and three answers on OpenAI's form (2026-09-26). The script's floating panel lives in a shadow root, because Ashby's styles squashed its lines on top of each other. A job without documents gets `resume.pdf` and no cover letter.
- Ready by morning: the Review list (`review_queue()`) shows only complete jobs, `ready()`: answers plus `docs` when `resume.json` exists and `tailor_docs` is on. Step 3 of the run, `get_ready()`, first fills in whatever the best `queue_size` candidates (`queue_candidates()`) are missing, then step 4 drafts answers for more matches (including boards the autofill can't submit on) with what's left of `max_drafts_per_run`. Both stop at `prep_deadline()`, 10 minutes before `digest_at`, so the list is settled when the digest goes out; a run started after the digest time has no deadline. A job whose model call fails is skipped and tried again next time. The Review list's "Get them ready now" button (`POST /api/jobs/ready`, `run_get_ready()`) runs step 3 alone, under the same lock and Stop button as a search. A ready job takes about 4 minutes of the 8B (answers about 2, resume summary and cover letter about 2).
- Hiring rules (2026-09-26, listed for Sai in `HIRING_RULES.md`; keep that file in step with `RULES` and `check_rules()`): `RULES` in `jobs.py` lists 20 checks an employer's own rules would trip (duplicates, reposts, company caps, role spread, pacing, cooldowns, interviewing or withdrawn elsewhere, graduation windows, citizenship, sponsorship, years, PhD, on-site, inconsistent answers or resumes to one company, AI-use policies, referrals and agencies, conflict answers). They're plain code over jobs.json, the profile and the form questions, not model judgment. `check_rules(job, rules_context())` returns flags with an action: block (left out of `queue_candidates()`, `review_queue()` and `next_approved()`, and `approve()` refuses), ask (`approve()` needs the rule in `allow`, stored as `rule_overrides`; the card's button becomes Approve anyway), warn (a note). Settings live in `meta["rules"]` (actions, the default cap of 3 in 30 days, per-company caps with Google at 3 in 90, cooldown days, referral and agency notes), edited under You, Rules (`GET/PUT /api/jobs/rules`). Rules run again right before submitting, since another application may have gone out after the approval. The graduation window compares months: "Fall 2026 or Spring 2027" is December 2026 to May 2027, a bare year is the whole year, and comparing years alone let Scale AI's form through for a May 2026 graduate. Caps a company states itself win (`stated_cap()` over the job's note and posting): Ashby's `applicationLimitCalloutHtml` (OpenAI: 5 in 180 days) is read with the form questions into the job's note, and `ensure_limit()` fetches it once for Ashby jobs prepared before that, at approval and in `get_ready()`. The graduation window also looks after emerging talent, early career, university grad and campus hire, since OpenAI's "Applied Emerging Talent (2027)" slipped past. Cooldowns need `interviewed`, set when a job reaches interview (panel or inbox); a rejection without it only asks. A job whose form has an AI-use policy (`ai_restricted()`) gets no model-written answers, summary or cover letter from `fill()` (`base` is the plain resume.pdf) until Sai approves it anyway. Status `withdrew` covers withdrawing or declining.
- Stopping a search: `POST /api/jobs/stop` sets `progress["stop"]`; the next `set_progress()` raises `Stopped`, which `run()` catches. Everything saved so far stays, and the date check means it won't start again the same day. Before this existed, the only way to stop a run was restarting agent-web.
